Shader /*ase_name*/ "Universal/UniversalPBR_Template_NewNew" /*end*/
{
	Properties
	{
		/*ase_props*/
	...
	SubShader
	{
		/*ase_subshader_options:Name=Additional Options
			...
			Option:Use Environment Reflections:false,true:true
				true:ShowOption:  Use Unity Default Reflections
				true:ShowOption:  Use Custom EnvMap
				false:HideOption:  Use Unity Default Reflections
				false:HideOption:  Use Custom EnvMap
			Option:  Use Unity Default Reflections:false,true:false
				true:RemoveDefine:pragma multi_compile _ENVIRONMENTREFLECTIONS_OFF
				false:SetDefine:pragma multi_compile _ENVIRONMENTREFLECTIONS_OFF
			Option:  Use Custom EnvMap:false,true:false
				true:SetDefine:pragma multi_compile _CUSTOMENVIRONMENTREFLECTIONS_ON

				false:RemoveDefine:pragma multi_compile _CUSTOMENVIRONMENTREFLECTIONS_ON
		...
		*/
		...
		Pass
		{
			...
			#include "Assets/ShaderLibrary/Lighting_Custom.hlsl"
			...
			struct VertexOutput
			{
				...
				#if defined(REQUIRES_VERTEX_SHADOW_COORD_INTERPOLATOR)	//Shadow
			...
			VertexOutput VertexFunction( VertexInput v /*ase_vert_input*/ )
			{
				...
				//Lighting中设置
				//若不想受到影响，可在ASE中取消勾选Built-in Fog
				#ifdef ASE_FOG
						...
						/*ase_frag_input*/ ) : SV_Target
			{
				...
				float4 Debug = /*ase_frag_out:Debug;Float4;12;-1;_Debug*/float4(0, 0, 0, 0)/*end*/;
				...
				half3 lightDirectionWS = normalize(_MainLightPosition).xyz;

				PBRInputData inputPBRData;
				InitializePBRInputData(inputData.positionWS, inputData.normalWS, inputData.viewDirectionWS, lightDirectionWS, inputPBRData);

				

				half4 color = PBR(inputPBRData, BaseColor, Metallic, 1 - Smoothness, Occlusion, Alpha);
